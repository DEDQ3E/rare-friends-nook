# Rare Friends: Friend Nook - local launcher for Windows.
...
param([int]$Port = 4183, [switch]$NoBrowser)
...
if (-not $listener) {
    Write-Host 'Could not open a local port (4183-4202 are busy).' -ForegroundColor Red
    Read-Host 'Press Enter to close'
    exit 1
}
...
Write-Host ''
Write-Host '  Rare Friends: Friend Nook' -ForegroundColor Green
